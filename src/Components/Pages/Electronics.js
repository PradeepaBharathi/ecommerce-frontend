...
function Electronics() {
  ...
  return (
    <div >
     ...
     <img src={electronics} className='w-11/12 h-96 m-14 xs:w-72  xs:m-5 sm:ml-10' alt="Men's Fashion" />
      <div className='absolute inset-0 flex items-center justify-center text-7xl text-amber-950 font-bold italic xs:text-2xl'>
        Gadgets
      </div>
     </div>
      <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3 justify-center mt-20 mb-10" >
        {products.map((product) => {
          return (
            <div key={product._id} className="group relative w-72  h-auto xs:ml-3 sm:ml-10 lg:w-72 lg:h-80 flex flex-col border p-4 rounded-lg shadow-md bg-amber-100" onClick={()=>handleProduct(product._id)}>
